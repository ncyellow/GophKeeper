// Package storage - implements read and write operations to the database for all main entities: Cards, Logins, etc.
package storage
...
// NewPgStorage - constructor for postgresql-based storage, not used directly, only through the factory
...
func (p *PgStorage) Register(ctx context.Context, user models.User) (int64, error) {
	...
	// since login is unique, any attempt to insert a duplicate login will result in an error
	if err != nil {
...
func (p *PgStorage) AddCard(ctx context.Context, userID int64, card models.Card) error {
	...
	log.Info().Msgf("New card with identifier - %s added. @card - %d", card.ID, lastInsertID)
	return nil
}
...
func (p *PgStorage) AddLogin(ctx context.Context, userID int64, login models.Login) error {
	...
	log.Info().Msgf("New login credentials with identifier - %s added. @login - %d", login.ID, lastInsertID)
	return nil
}
...
func (p *PgStorage) AddText(ctx context.Context, userID int64, text models.Text) error {
	...
	log.Info().Msgf("New text data with identifier - %s added. @login - %d", text.ID, lastInsertID)
	return nil
}
...
func (p *PgStorage) AddBinary(ctx context.Context, userID int64, binData models.Binary) error {
	...
	log.Info().Msgf("New binary data with identifier - %s added. @bin - %d",
